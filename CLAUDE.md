...
**Live scores on the public site**: the tracker pushes derived totals to `admin-game-stats` about 4s after the last tap (debounced, and again on undo/redo), showing a status pill in its header; `js/app.js` re-reads the season every 15s while the tab is visible, plus immediately on focus, and repaints only when `liveFingerprint()` moves. It holds off while a box score or the nav drawer is open — the change is picked up on the next tick instead. This is polling, not push: a visitor sees a basket within ~15s. Neither side needs a new Edge Function or migration.
