...
export function scoresFingerprint(rows) {
  return (rows || [])
    .map(g => {
      ...
      // Status and the clock's anchor matter too: starting, pausing or ending
      // a game changes what a viewer sees without any score moving.
      const status = g.status ?? '';
      const clock = `${g.period ?? ''}/${g.clock_seconds ?? ''}/${g.clock_running ? 1 : 0}/${g.clock_updated_at ?? ''}`;
      return `${id}:${home}-${away}:${forfeit || ''}:${status}:${clock}`;
    })
    .sort()
    .join('|');
}
...
export function liveFingerprint(data) {
  ...
  // Built on scoresFingerprint rather than repeating it: the probe decides
  // *whether* to re-read using that one, and this decides whether to repaint.
  // When they disagreed, a change the probe caught — a game going live, a
  // period ending, the final whistle — was re-read and then silently dropped.
  const scores = scoresFingerprint(data.scores);
